...
入口复制到浏览器：
https://lzdz1-isv.isvjd.com/dingzhi/shop/league/activity/7795914?activityId=dz211026100001616201
...
function followShop() {
    return new Promise(resolve => {
        let body = `activityId=dz211026100001616201&pin=${encodeURIComponent($.myPingData.secretPin)}&actorUuid=${$.actorUuid}&taskType=23&taskValue=1000009621&shareUuid=${$.shareUuid ? $.shareUuid : 'f2700876d9b5416ba3e0b683f1f83db8'}`
...
function getDrawRecordHasCoupon() {
    return new Promise(resolve => {
        let body = `activityId=dz211026100001616201&actorUuid=${$.actorUuid}&shareUuid=${$.shareUuid ? $.shareUuid : 'f2700876d9b5416ba3e0b683f1f83db8'}&pin=${encodeURIComponent($.myPingData.secretPin)}`
...
function saveTask(taskType,taskValue) {
    return new Promise(resolve => {
        let body = `activityId=dz211026100001616201&pin=${encodeURIComponent($.myPingData.secretPin)}&taskType=${taskType}&taskValue=${taskValue}&actorUuid=${$.actorUuid}&shareUuid=${$.shareUuid ? $.shareUuid : 'f2700876d9b5416ba3e0b683f1f83db8'}`
...
async function drawContent() {
    return new Promise(resolve => {
        let body = `activityId=dz211026100001616201&pin=${encodeURIComponent($.myPingData.secretPin)}`
        $.post(taskPostUrl('/dingzhi/taskact/common/drawContent', body, `https://lzdz1-isv.isvjcloud.com/dingzhi/shop/league/activity/805745?activityId=dz211026100001616201&shareUuid=${$.shareUuid ? $.shareUuid : 'f2700876d9b5416ba3e0b683f1f83db8'}`), async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${$.name} API请求失败，请检查网路重试`)
                } else {
...
async function startDraw(type) {
    return new Promise(resolve => {
        let body = `activityId=dz211026100001616201&actorUuid=${$.actorUuid}&shareUuid=${$.shareUuid ? $.shareUuid : 'f2700876d9b5416ba3e0b683f1f83db8'}&pin=${encodeURIComponent($.myPingData.secretPin)}&type=${type}`
        $.post(taskPostUrl('/dingzhi/shop/league/startDraw', body, `https://lzdz1-isv.isvjcloud.com/dingzhi/shop/league/activity/805745?activityId=dz211026100001616201&shareUuid=${$.shareUuid ? $.shareUuid : 'f2700876d9b5416ba3e0b683f1f83db8'}`), async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${$.name} API请求失败，请检查网路重试`)
                } else {
...
function getHtml() {
    return new Promise(resolve => {
        $.get({
            url: `https://lzdz1-isv.isvjcloud.com/dingzhi/shop/league/activity/5564548?activityId=dz211026100001616201&shareUuid=${$.shareUuid ? $.shareUuid : 'f2700876d9b5416ba3e0b683f1f83db8'}&adsource=null&shareuserid4minipg=P0CZ6sYjxiDL7YQZAjODCU7oeVP9kq2pYSH90mYt4m3fwcJlClpxrfmVYaGKuquQkdK3rLBQpEQH9V4tdrrh0w==&shopid=1000009621&lng=113.387958&lat=22.511013&sid=09fdc8e908526b5538a4ad4a265f40dw&un_area=19_1657_52093_0`,
            headers: {
...
function adLog() {
    return new Promise(resolve => {
        $.post({
            ...
            body: `venderId=1000009621&code=99&pin=${encodeURIComponent($.myPingData.secretPin)}&activityId=dz211026100001616201&pageUrl=https://lzdz1-isv.isvjcloud.com/dingzhi/shop/league/activity/797702?activityId=dz211026100001616201&shareUuid=${$.shareUuid ? $.shareUuid : 'f2700876d9b5416ba3e0b683f1f83db8'}&adsource=null&shareuserid4minipg=P0CZ6sYjxiDL7YQZAjODCU7oeVP9kq2pYSH90mYt4m3fwcJlClpxrfmVYaGKuquQkdK3rLBQpEQH9V4tdrrh0w==&shopid=1000009621&lng=113.387958&lat=22.511013&sid=09fdc8e908526b5538a4ad4a265f40dw&un_area=19_1657_52093_0`,
            headers: {
                'User-Agent': `Mozilla/5.0 (Linux; U; Android 8.0.0; zh-cn; Mi Note 2 Build/OPR1.170623.032) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/61.0.3163.128 Mobile Safari/537.36 XiaoMi/MiuiBrowser/10.1.1`,
                'Host': 'lzdz1-isv.isvjcloud.com',
...
function getActorUuid() {
    return new Promise(resolve => {
        $.post({
            ...
            body: `activityId=dz211026100001616201&pin=${encodeURIComponent($.myPingData.secretPin)}&pinImg=https%3A%2F%2Fimg10.360buyimg.com%2Fimgzone%2Fjfs%2Ft1%2F7020%2F27%2F13511%2F6142%2F5c5138d8E4df2e764%2F5a1216a3a5043c5d.png&nick=${encodeURIComponent($.myPingData.nickname)}&cjyxPin=&cjhyPin=&shareUuid=${$.shareUuid ? $.shareUuid : 'f2700876d9b5416ba3e0b683f1f83db8'}`,
            headers: {
                'User-Agent': `Mozilla/5.0 (Linux; U; Android 8.0.0; zh-cn; Mi Note 2 Build/OPR1.170623.032) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/61.0.3163.128 Mobile Safari/537.36 XiaoMi/MiuiBrowser/10.1.1`,
                'Host': 'lzdz1-isv.isvjcloud.com',
                'Content-Type': 'application/x-www-form-urlencoded; Charset=UTF-8',
...
function taskPostUrl(url, body, referer) {
    return {
        ...
        headers: {
            ...
            "Referer": referer ? referer : `https://lzdz1-isv.isvjcloud.com/lzclient/dz/2021jan/eliminateGame/0713eliminate/?activityId=735c30216dc640638ceb6e63ff6d8b17&shareUuid=${$.shareUuid ? $.shareUuid : 'f2700876d9b5416ba3e0b683f1f83db8'}&adsource=&shareuserid4minipg=u%2FcWHIy7%2Fx3Ij%20HjfbnnePkaL5GGqMTUc8u%2Fotw2E%20a7Ak3lgFoFQlZmf45w8Jzw&shopid=0&lng=114.062541&lat=29.541254&sid=57b59835c68ed8959d124d644f61c58w&un_area=4_48201_54794_0`,
            "User-Agent": UA,
